    ...
    for i, (old_folder, old_index, new_index) in enumerate(episode_mapping):
        ...
        if source_path:
            try:
                ...
                print(f"parquet columns {df.columns.tolist()} source_path {source_path}")

                # 检查是否需要填充维度 - 为不同特征类型使用不同的最大维度
                # 为状态向量填充
                if "state" in df.columns:
                    # 检查第一个非空值 (Check first non-null value)
                    for _idx, value in enumerate(df["state"]):
...
def pad_parquet_data(source_path, target_path, original_dim=14, target_dim=18):
    ...
    if "state" in df.columns:
        ...
        print(f"First state type: {type(first_state)}, value: {first_state}")
        ...
        if isinstance(first_state, (list, np.ndarray)):
            ...
            if state_dim < target_dim:
                # 填充向量
                print(f"Padding state from {state_dim} to {target_dim} dimensions")
                new_df["state"] = df["state"].apply(
...
def copy_images(source_folders, output_folder, episode_mapping, default_fps=20, fps=None):
    ...
    for feature_name, feature_info in info["features"].items():
        if feature_info.get("dtype") == "video" or feature_info.get("dtype") == "image":
            image_keys.append(feature_name)
...
def merge_datasets(
    ...
    if "features" in info:
        ...
        # 更新状态向量维度
        if "state" in info["features"] and "shape" in info["features"]["state"]:
            info["features"]["state"]["shape"] = [actual_state_max_dim]
            print(f"Updated state shape to {actual_state_max_dim}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--state_max_dim", type=int, default=7, help="Maximum state vector dimension (default: 32)")
    parser.add_argument("--action_max_dim", type=int, default=7, help="Maximum action vector dimension (default: 32)")
